...
namespace Microsoft.Azure.WebJobs.ServiceBusSessions.Config
{
    /// <summary>
    /// Extension configuration provider used to register ServiceBus triggers and binders
    /// </summary>
    [Extension("ServiceBus")]
    internal class ServiceBusSessionExtensionConfigProvider : IExtensionConfigProvider
    {
        ...
        public void Initialize(ExtensionConfigContext context)
        {
            ...
            ServiceBusSessionTriggerAttributeBindingProvider triggerBindingProvider = new ServiceBusSessionTriggerAttributeBindingProvider(_nameResolver, _options, _sessionProvider, _configuration);
            context.AddBindingRule<ServiceBusSessionTriggerAttribute>().BindToTrigger(triggerBindingProvider);                        
        }
        ...
        internal static void LogExceptionReceivedEvent(ExceptionReceivedEventArgs e, ILoggerFactory loggerFactory)
        {
            try
            {
                ...
                string message = $"IQueueClient error (Action={ctxt.Action}, ClientId={ctxt.ClientId}, EntityPath={ctxt.EntityPath}, Endpoint={ctxt.Endpoint})";
